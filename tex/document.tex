\documentclass[paper=6in:9in,pagesize=pdftex,headinclude=on,footinclude=on,11pt]{scrbook}
...
\section{Entrée (\emph{Check In})} \label{entree}

Commencez une réunion par une \emph{Entrée}. Vous pouvez également en faire une en tête à tête ou en groupe chaque fois que
...
\section{Sortie (\emph{Check Out})} \label{sortie}
...
respecter ces engagements ou que vous pensez être plus utile ailleurs, vous devez annoncer la \emph{Sortie}.
...
\begin{itemize}
	...
	\item Assurez-vous de vouloir résoudre paisiblement tout conflit potentiel avant de contrôler l'intention de qui que ce soit.
	      Si vous n'avez pas d'intention pacifique, sortez (\fullref{sortie}).
...
\subsection{Remarques}
\begin{itemize}
	\item Si le conflit qui émerge semble impossible à résoudre, sortez (\fullref{sortie}) et demandez de l'aide (\fullref{demande-aide}).
\end{itemize}
...
\begin{enumerate}
	...
	\item Le facilitateur précise qu'il s'agit d'une version \og{}express\fg{} d'une Formation aux Protocoles Fondamentaux (\emph{Core Protocols BootCamp}). Cette
          ...
          supplémentaires en vue d'atteindre des résultats similaires
...
\begin{itemize}
	...
	\item Former des petits groupes -- de cinq personnes ou moins -- marche mieux pour vous aligner sur votre Vision Partagée.
\end{itemize}
